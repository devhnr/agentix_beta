<?php
	...
	class Talk_to_expert extends CI_Controller {
	private $_data = array();
	function __construct()
	{
		...
		$this->load->model('talk_to_expert_model');
	...
	function lists()
	{
		...
		$config['base_url'] = $url_to_paging.'talk_to_expert/lists/';
		...
		$return = $this->talk_to_expert_model->lists($config['per_page'],$this->uri->segment(3), $data);
		$data['result'] = $return['result'];
		$config['total_rows'] = $return['count'];
		//echo "<pre>";print_r($data);break;
		$this->pagination->initialize($config);
		$this->load->view('list_talk_to_expert', $data);
	}
	...
	{	 	
			if(is_numeric($id))
			{
				$result = $this->talk_to_expert_model->get_user_data($id);  
				...
				if($this->input->post('action') == 'edit_talk_to_expert') 
				{
					...
					{
							$this->talk_to_expert_model->edit($id, $form_field);
							$this->session->set_flashdata('L_strErrorMessage','Talk To Expert Updated Successfully');
							redirect($this->config->item('base_url').'talk_to_expert/lists');
					}
				}
				$this->load->view('edit_talk_to_expert',$data);
			} 
			else 
			{
	...
	function deletes()
	{
		if(isset($_POST['selected']) && count($_POST['selected']) > 0) {
			...
			foreach($_POST['selected'] as $selCheck) {
				...
				if($this->talk_to_expert_model->deletes($selCheck)) {
					
					$log_data = array(
									'username' => $this->session->userdata('username'),
									'login_user_id' => $this->session->userdata('adminId'),
									'module_name' => 'Delete Talk To Expert',
									'corporate_name' => '',
									'policy_number' => '',
					...
					$this->session->set_flashdata('L_strErrorMessage','Talk To Expert Deleted Successfully!!!!');
				}  
				else 
				{
	...
	function userstatus($redirect,$id,$value)	{
			...
			$result=$this->talk_to_expert_model->updatestatus($id,$value);
	...
	function download_user(){
		...
		$orders_list = $this->talk_to_expert_model->all_user();
	...
	public function createExcel() {
		$quateData = $this->talk_to_expert_model->get_book_consultation_list();
        ...
        $writer = new Xlsx($spreadsheet);
			...
			//   header('Content-Disposition: attachment;filename="TalkToExpert.xls"');
