...
class Summary extends CI_Controller {
    ...
    public function showClaimStatusSummary(){
        ...
        $output = '';
        if(!empty($keys)){
            $output.= '<tr><td>'.$keys[0].'</td><td>'.number_format($claim_status_summary['closed_cashless']).'</td><td>'.number_format($claim_status_summary['closed_cashless_claimed']).'</td><td>'.number_format($claim_status_summary['closed_cashless_incurred']).'</td><td>'.number_format($claim_status_summary['closed_reimbursement']).'</td><td>'.number_format($claim_status_summary['closed_reimbursement_claimed']).'</td><td>'.number_format($claim_status_summary['closed_reimbursement_incurred']).'</td>
            ...
            $output.= '<tr><td>'.$keys[1].'</td><td>'.number_format($claim_status_summary['outstanding_cashless']).'</td><td>'.number_format($claim_status_summary['outstanding_cashless_claimed']).'</td><td>'.number_format($claim_status_summary['outstanding_cashless_incurred']).'</td><td>'.number_format($claim_status_summary['outstanding_reimbursement']).'</td><td>'.number_format($claim_status_summary['outstanding_reimbursement_claimed']).'</td><td>'.number_format($claim_status_summary['outstanding_reimbursement_incurred']).'</td>
            ...
            $output.= '<tr><td>'.$keys[2].'</td><td>'.number_format($claim_status_summary['paid_cashless']).'</td><td>'.number_format($claim_status_summary['paid_cashless_claimed']).'</td><td>'.number_format($claim_status_summary['paid_cashless_incurred']).'</td><td>'.number_format($claim_status_summary['paid_reimbursement']).'</td><td>'.number_format($claim_status_summary['paid_reimbursement_claimed']).'</td><td>'.number_format($claim_status_summary['paid_reimbursement_incurred']).'</td>
            ...
            $output.= '<tr><td>'.$keys[3].'</td><td>'.number_format($claim_status_summary['rejected_cashless']).'</td><td>'.number_format($claim_status_summary['rejected_cashless_claimed']).'</td><td>'.number_format($claim_status_summary['rejected_cashless_incurred']).'</td><td>'.number_format($claim_status_summary['rejected_reimbursement']).'</td><td>'.number_format($claim_status_summary['rejected_reimbursement_claimed']).'</td><td>'.number_format($claim_status_summary['rejected_reimbursement_incurred']).'</td>
    ...
    public function showSettledACS(){
        ...
        if(!empty($settled_acs['paid_claim_count'])){
          ...
          $output.= '<tr><td>'.$keys[0].'</td><td>'.number_format($settled_acs['claim_count']).'</td><td>'.number_format($settled_acs['paid_claim_count']).'</td><td>₹ '.number_format($settled_acs['claim_paid_amount']).'</td><td>'.number_format($cashless_acs, 2, '.', '').'</td>
          <td>'.number_format($settled_acs['claim_reimbursement_count']).'</td><td>'.number_format($settled_acs['paid_claim_reimbursement_count']).'</td><td>₹ '.number_format($settled_acs['claim_paid_reimbursement_amount']).'</td><td>'.number_format($reimbursement_acs, 2, '.', '').'</td>
          <td>'.(number_format($settled_acs['claim_count'] + $settled_acs['claim_reimbursement_count'])).'</td><td>'.(number_format($settled_acs['paid_claim_count'] +$settled_acs['paid_claim_reimbursement_count'])).'</td><td>₹ '.(number_format($settled_acs['claim_paid_amount']+$settled_acs['claim_paid_reimbursement_amount'])).'</td><td>'.number_format($total_acs, 2, '.', '').'</td></tr>';
        }
    ...
    public function showClaimTATSummary(){
        ...
        if(!empty($keys)){
            ...
            for($i=0;$i<count($keys);$i++){
              ...
              $output.= '<tr><td>'.$keys[$i].' days</td><td>'.number_format($tat_summary['tat_count_'.$i]).'</td><td>'.number_format($percentage).'</td></tr>';
            }
        }
        $res = array(
            'body_html' => $output,
            'total_val' =>  number_format($total_clims),
            'total_val1' =>  number_format($grand_total_percentage),
        );
        echo json_encode($res);
    ...
    public function showRelationWiseSummary(){
        ...
        if(!empty($keys)){
          for($i=0;$i<count($keys);$i++){
            ...
            $output.= '<tr><td>'.$keys[$i].'</td><td>'.number_format($ralation_summary['claim_count_'.$i]).'</td><td>'.number_format($ralation_summary['paid_claim_'.$i]).'</td><td>'.number_format($ralation_summary['settled_amt_'.$i]).'</td><td>'.number_format($percentage, 2, '.', '').'</td><td>'.number_format($acs).'</td></tr>';
    ...
    public function showNetworkWiseSummary(){
        ...
        if(!empty($keys)){
            ...
            $output.= '<tr><td rowspan="4">'.$keys[0].'</td><tr><td>Cashless</td><td>'.number_format($network['cashless_count']).'</td><td>'.number_format($network['paid_cashless']).'</td><td>'.number_format($network['settled_cashless']).'</td><td>'.number_format($acs1, 2, '.', '').'</td><td rowspan="4" style="padding: 4.9% !important;">'.number_format($total_percentage1, 2, '.', '').'</td></tr>
            ...
            <td>'.($network['paid_cashless']+$network['paid_reimbursement']).'</td><td>'.number_format(($network['settled_cashless']+$network['settled_reimbursement'])).'</td><td>'.number_format($acs3, 2, '.', '').'</td></tr></tr>';

            $output.= '<tr><td rowspan="4">'.$keys[1].'</td><tr><td>Cashless</td><td>'.number_format($network['cashless_count_1']).'</td><td>'.number_format($network['paid_cashless_1']).'</td><td>'.number_format($network['settled_cashless_1']).'</td><td>'.number_format($acs4, 2, '.', '').'</td><td rowspan="4" style="padding: 4.9% !important;">'.number_format($total_percentage2, 2, '.', '').'</td></tr>
        ...
        $res = array(
            ...
            'total_val1' =>  number_format($paid_clims),
            'total_val2' =>  number_format($settled_amt),
            'total_val3' =>  number_format($total_acs),
