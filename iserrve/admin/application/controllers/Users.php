...
class Users extends CI_Controller {
...

	{
...

					{
...

						{
							...
							$log_data = array(
								...
								'module_name' => 'Create Admin Users',
								'corporate_name' => '',
								'policy_number' => '',
		...
		$data['all_corp'] = $this->users_model->all_corporator();
...

	{
		...
		$data['name'] = $this->admin->xss_clean_inputData($this->input->post('name'));
			...
			foreach($_POST['selected'] as $selCheck) {
				...
				if($this->users_model->deletes($selCheck)) {
					...
					$log_data = array(
						'username' => $this->session->userdata('username'),
						'login_user_id' => $this->session->userdata('adminId'),
						'module_name' => 'Delete Admin Users',
						'corporate_name' => '',
						'policy_number' => '',
						'created_at' => date('Y-m-d h:i:sa')
					);
	...
	function check_user_email()
	{
		 // echo "test";exit;
		$email = $_POST['email'];
		$data = $this->users_model->check_user_email($email);
		if($data != ""){
			echo "0";
			die;
		}
	...
	function check_user_name()
	{
		 // echo "test";exit;
		$login = $_POST['login'];
		$data = $this->users_model->check_user_name($login);
		if($data != ""){
			echo "0";
			die;
		}
