...
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;
	...
	class Home_get_quote extends CI_Controller {
	private $_data = array();
	function __construct()
	{
		...
		$this->load->model('home_get_quote_model');
	...
	function lists()
	{
		...
		$return = $this->home_get_quote_model->lists($config['per_page'],$this->uri->segment(3), $data);
		$data['result'] = $return['result'];
		$config['total_rows'] = $return['count'];
		//echo "<pre>";print_r($data);break;
		$this->pagination->initialize($config);
		$this->load->view('list_home_quote', $data);
	}
	...
	function edit($id)
	{
			if(is_numeric($id))
			{
				$result = $this->home_get_quote_model->get_user_data($id);
				...
				if($this->input->post('action') == 'edit_user')
				{
					...
					{
							$this->home_get_quote_model->edit($id, $form_field);
	...
	function deletes()
	{
		if(isset($_POST['selected']) && count($_POST['selected']) > 0) {
			...
			foreach($_POST['selected'] as $selCheck) {
				...
				if($this->home_get_quote_model->deletes($selCheck)) {
	...
	function userstatus($redirect,$id,$value)	{
			...
			$result=$this->home_get_quote_model->updatestatus($id,$value);
	...
	function download_user(){
		...
		$orders_list = $this->home_get_quote_model->all_user();
	...
	public function createExcel() {
			$quateData = $this->home_get_quote_model->get_home_quote_list();
			...
			$sheet->setCellValue('G1', 'No. of Employees');
			...
			foreach ($quateData as $val){
				...
				$sheet->setCellValue('G' . $rows, $val['number_of_employee']);
