			...
			<INPUT TYPE="hidden" NAME="action" VALUE="add_users">
               ...
               <div class="form-group">
					...
					<div class="form-group">
						...
						<label  for="select">Corporate <span class="mandatory_field">*</span></label>

						<div class="">

                           <select id="corp_id_old" name="corp_id[]" class="form-control" multiple="multiple" >

								<?php 

								if($all_corp != '' && count($all_corp) > 0){

								for($i=0;$i<count($all_corp);$i++){?>

									<option value='<?php echo $all_corp[$i]->id; ?>' ><?php echo $all_corp[$i]->co_name; ?></option>
				...
				<div class="form-group">
					...
					<label  for="inputEmail">Confirm Password: <span class="mandatory_field">*</span></label>
...
$('#corp_id').mediaBoxes('select_all');
$('#corp_id').mediaBoxes('deselect_all');
...
<script>
	...
	function validate(){
	    ...
	    var url= '<?php echo $base_url;?>users/check_user_name';
        // alert(url);
        $.ajax({
            url:url,
            type: 'post',
            ...
            success:function(msg){
                if(msg == "0"){
                    ...
                    return false;
					    ...
					    var url= '<?php echo $base_url;?>users/check_user_email';
					         // alert(url);
					        $.ajax({
					            url:url,
					            type: 'post',
					            ...
					            success:function(msg){
					                if(msg == "0"){
					                    $("#error_msg1").html("User Email Already Exists.");
					                    $("#validator").css("display", "block");
					                    $('html, body').animate({'scrollTop': $("#error_msg1").position().top});
					                    // jQuery("#group_code").val('')
					                    return false;
