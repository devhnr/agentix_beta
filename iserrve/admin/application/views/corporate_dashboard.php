...
.v-box-layout {
  ...
  border-radius: 4px;
  padding: 5px 10px;
  box-shadow: 0 3px 10px rgba(204, 203, 203, 0.2);
  margin-bottom: 10px;
  padding: 10px 10px;
}
...
<div id="home" class="tab-pane v-tabular fade in active">
      ...
      <div class="col-lg-3">
            ...
            <select class="form-control select_sum_insured" name="" onchange="change_sum_insured('<?php echo $corporate_id; ?>','<?php echo $policy_id; ?>',this.value);">
      ...
      ?>
         <div class="col-lg-4">
            <div class="v-card">
               ...
               <p><? echo $answer_data->field_type;?></p>

               <?php    if($answer_data->field_type == 'Yes' || $answer_data->field_type == 'yes'){
         ...
         $other_benefit = $this->corporate_dashboard_model->get_other_benefit($corporate_id,$policy_id);
         
         //echo "<pre>";print_r($other_benefit);echo"</pre>";
       ...
       ?>
        ...
        <div class="col-lg-4">
            <div class="v-card">
               ...
               <div class="v-card-cont">
            <p><?php echo $other_benefit->inclusions?></p>
            </div>
            </div>
            </div>
         <?php } ?>
               ...
               <?php
                  ...
                  if($enrolement_details != ''){
                     ...
                     foreach($enrolement_details as $enrolement_details_data){
                        ...
                        //echo "<pre>";print_r($enrolement_details_data);echo"</pre>";
                        $im = 1 ;
                        foreach($enrolement_att as $enrolement_att_data){
                           ...
                           $ecard_data = $this->corporate_dashboard_model->get_e_card_dev($policy_data->id,$enrolement_att_data->employee_id);

                  $policy_data_new = $this->corporate_dashboard_model->get_policy_data($ecard_data->policy_id);
               ...
               <tr>
                  ...
                  <td>
                     ...
                     //echo "sss";
                       ...
                       <button type="button" class="btn bg-purple2 download_ecard " data-policy_id="<?php echo $policy_data->id ?>" data-id="<?php echo $enrolement_att_data->employee_id; ?>" style="color:white;">E-Card</button>
...
<div id="menu9" class="tab-pane v-tabular fade in">
      ...
      <!--<form action="<?php //echo $base_url; ?>home/generatePDF" method="post">

      <input type="hidden" name="pdf_corp_id" value="<?php //echo $corporate_id;?>" >

      <input type="hidden" name="pdf_policy_id" value="<?php //echo $policy_id;?>" >

      <input type="hidden" name="pdf_policy_start_date" id="pdf_policy_start_date" class="form-control" value="<?php //echo $policy_data->policy_start_date ?>" <?php //echo $read_only; ?>>

      <input type="hidden" name="pdf_policy_end_date" id="pdf_policy_end_date" class="form-control" value="<?php //echo $policy_data->policy_end_date ?>" <?php //echo $read_only; ?>>
            ...
            <div class="form-group col-sm-6">
               ...
               <select id="tpa" name="tpa" class="form-control">
                   ...
                   <?php foreach(TPA as $TPA){ ?>
                     <option value="<?=$TPA?>" <?php if($policy_data->tpa == $TPA){echo "selected"; } ?>><?=$TPA?></option>
                   <?php } ?>
               </select>
           </div>
            <div class="form-group col-sm-6">
...
<script type="text/javascript">
  $(document).ready(function() {
      ...
      var corporate_id = $("input[name=corporate_id]").val();
        ...
        getPolicyFeatures(policy_id,corporate_id);
                ...
                success: function(data){
                    ...
                    if(data.premium != ''){
                        ...
                        if(data.earned_premium != ''){
                           var t = addCommas(premium.second.total_premium);
                          $('.net_permium').html('₹ '+ t);
...
<script type="text/javascript">
 ...
 $(document).on('click', '.download_ecard', function(){
            ...
            $.ajax({
                url : '<?php echo $base_url; ?>home/download_ecard_new',
                type : 'post',
        ...
        function getPolicyFeatures(policy_id,corporate_id){
            $.ajax({
                url : '<?php echo $base_url; ?>home/getSumInsured',
                type : 'post',
                data : 'policy_id=' + policy_id + "&corporate_id=" + corporate_id,
                dataType: 'json',
                success: function(data){
                    var currentURL = $(location).attr('href');
                    var pieces = currentURL.split("?sum_insured=");
                    if(data != ''){
                      var html = '<option value="" selected readonly>--Select sum Insured--</option>';
                      var i;
                      for(i=0; i<data.length; i++){
                          html += '<option value='+data[i].id;
                          if(pieces[1] == data[i].id){
                            html += ' selected';
                          }
                          html += '>'+data[i].sum_insured+'</option>';
                      }
    ...
    //      var conf = confirm("Do you want to Change Sum Insured?");
    //      if (conf == true) {
    //          var base_url = $(location).attr('href');
    //          window.location = base_url + "?sum_insured=" + $(this).val();
    //      } else {
    //          return false;
    //      }
    // });

    function change_sum_insured(corporate_id,policy_id){
        var sum_insured = $(".select_sum_insured option:selected").val();
         var conf = confirm("Do you want to Change Sum Insured?");
            if (conf == true) {
                var base_url = '<?php echo $base_url. 'home/corporate_dashboard';?>';
                window.location = base_url + "/" + corporate_id + "/"+policy_id+ "?sum_insured=" + sum_insured;
            } else {
                return false;
            }
    }
