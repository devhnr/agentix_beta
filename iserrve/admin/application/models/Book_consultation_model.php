<?php
	class Book_consultation_model extends CI_Model {
	private $_data = array();
	function __construct() {
	  ...
	  function get_user($id){
		  ...
		  $this->db->where('id = ',$id);
		  $query = $this->db->get('book_consultation');	
		  if ($query->num_rows() > 0)	
		  {	
		  	$result = $query->result();	
		  	return $result;	
 ...
 function lists($num, $offset, $content) 
	{
		...
		$sql = "SELECT * FROM book_consultation where id <> 0";
		if($num!='' || $offset!='')
		{
		...
		$sql_count = "SELECT * FROM  book_consultation  WHERE id <> 0";
	...
	{
 		$this->db->where('id = ',$id);
		if ($this->db->delete('book_consultation'))	{
			return true;
		} else {
...

	{
		...
		if ($this->db->update('book_consultation', $this->_data))	{
	...
	function all_book_consultation(){
	...
	public function get_book_consultation_list(){
        try{
            $this->db->select('id,name,company,email,phone,location,DATE_FORMAT(added_date, "%d-%m-%Y") AS created_at');
            $this->db->from('book_consultation');
