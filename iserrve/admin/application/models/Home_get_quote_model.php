<?php
	class Home_get_quote_model extends CI_Model {
	private $_data = array();
 ...
 function lists($num, $offset, $content)
	{
		...
		$sql = "SELECT * FROM home_get_quote where id <> 0";
		if($num!='' || $offset!='')
		{
	...
	public function get_home_quote_list(){
        ...
        try{
            ...
            $this->db->select('id,name,company_name,email,phone,location,number_of_employee,send_me_whats_app,DATE_FORMAT(added_date, "%d-%m-%Y") AS created_at');

            $this->db->from('home_get_quote');
