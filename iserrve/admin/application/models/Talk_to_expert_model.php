<?php
	class Talk_to_expert_model extends CI_Model {
	private $_data = array();
 ...
 function lists($num, $offset, $content) 
	{
		...
		$sql = "SELECT * FROM need_help where id <> 0";
		if($num!='' || $offset!='')
		{
		...
		$sql_count = "SELECT * FROM  need_help  WHERE id <> 0";
...

	{
		...
		if ($this->db->update('need_help', $this->_data))	{
	...
	public function get_book_consultation_list(){
        ...
        try{
            $this->db->select('id,name,company,email,phone_number,DATE_FORMAT(added_date, "%d-%m-%Y") AS created_at');
            $this->db->from('need_help');
