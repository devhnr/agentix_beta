...
<section class="padding-25px-tb bg-light-gray">
    <div class="container">
        <div class="row get-quote align-items-center margin-30px-bottom">
			<div class="col-lg-4">
				<a href="https://www.raghnall.co.in/msmeaccelerate/beta" class="v-btn">BACK TO HOME</a>
			</div>
  ...
  <div class="tab" id="hide">
      ...
      <p>We will only use it to verify number. No Spam, we promise!</p>
			...
			<input type="checkbox" class="v-check" name="termscondition"><span>I Confirm That I Have Read, Understood And Agree To The Terms & Conditions.
  ...
  <span class="v-tooltiptext">I hereby authorize Raghnall Broking and Risk Management Pvt. Ltd. and all of its affiliates, group companies, and related parties to access the details such as my name, address, telephone number, e-mail address, birth date, and/or anniversary date shared by me, and contact me to provide information on the various products and services offered. I understand that this consent will override my NDNC registration if any.  I also understand that if at any point in time I wish to stop receiving such communications from Raghnall, I can withdraw my consent at www.raghnall.co.in.</span>
      ...
      <div class="form-group">
        <div class="row" style="margin-top: 20px;">
            <div class="col-md-12">
                <span id="phone_error" class="error alert-message valierror"
                    style="display:none;"></span>
                <span id="phone_success" class="successmain alert-message"
                    style="display:none;"></span>
            </div>
        </div>
    </div>
...
<p class="text-center">Our experts will get in touch with you for detailed quotes!</p>
                ...
                <div class="col-lg-12 col-12 padding-30px-bottom order-lg-2">
                    <div class="accountdetails">
						...
						<div class="d-flex align-items-center">
			<input type="checkbox" class="v-check " name="termscondition"><span>I would like to customize plan
...
function showTab(n) {
  ...
  if (n == (x.length - 1)) {
    document.getElementById("nextBtn").innerHTML = "BUY NOW";
  } else {
...
function nextPrev(n) {
  ...
  if (n == 1 && !validateForm() && !validateFormlNew()) return false ;
  // Hide the current tab:
  x[currentTab].style.display = "none";
  // Increase or decrease the current tab by 1:
  currentTab = currentTab + n;
  // if you have reached the end of the form...
  validateFormlNew(currentTab);
...
function validateFormlNew(currentTab){
    //alert('test');

        var company = $("#company").val();
        if (company == '') {
            $("#contact_error").html("Please Enter Company Name.");
        ...
        if (no_emp == '') {
        $("#contact_error").html("Please Enter No Of Employees.");
        $('#contact_error').show().delay(0).fadeIn('show');
        $('#contact_error').show().delay(2000).fadeOut('show');
        return false;
        }
     ...
     var name = $("#name").val();
        if (name == '') {
        $("#contact_error").html("Please Enter Your Name.");
        $('#contact_error').show().delay(0).fadeIn('show');
        $('#contact_error').show().delay(2000).fadeOut('show');
        return false;
     }
     ...
     var age_emp = $("#age_emp").val();
        if (age_emp == '') {
        $("#contact_error").html("Please Select Average Age Of Employees.");
        $('#contact_error').show().delay(0).fadeIn('show');
        $('#contact_error').show().delay(2000).fadeOut('show');
        return false;
     }
     ...
     if (coverage.length == 0) {
         jQuery('#contact_error').html("Plesae Choose Coverage");
        jQuery('#contact_error').show().delay(0).fadeIn('show');
        jQuery('#contact_error').show().delay(2000).fadeOut('show');
        return false;
     }
    ...
    if(phone != ''){
        ...
        if (!filter.test(phone)) {
            jQuery('#phone_error').html("Enter Valid Mobile Number");
            jQuery('#phone_error').show().delay(0).fadeIn('show');
            jQuery('#phone_error').show().delay(2000).fadeOut('show');
            return false;
        }
    ...
    var sum_value = sum_insurance_val * no_emp * 1.5/100;
