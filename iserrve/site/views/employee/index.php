        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="slick-assess">
                    ...
                    <div>
                        <a href="#" class="assess-banner" data-bs-toggle="modal" data-bs-target="#assessment1">
...
<div class="modal fade" id="assessment1" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
                <form class="v-form form-assessment" method="post" action="">
                    <div class="modal-contain">
                        <div class="row align-items-center">
                            ...
                            <div class="col-lg-12">
                                <label class="select" for="slct">
                                    <span class="disabled-label">Your Name *</span>
                                    <input type="text" class="custominput12" name="name" placeholder="Enter Name" id="assessment_name1" onkeypress="return onlyAlphabets(event,this);" />
                                    <p class="error m-0" id="name_err1"></p>
                                </label>
                            </div>
                            <div class="col-lg-12">
                                <label class="select" for="slct">
                                    <span class="disabled-label">Your Email Address *</span>
                                    <input type="text" class="custominput12" name="email" placeholder="Enter Email" id="assessment_email1" />
                                    <p class="error m-0" id="email_err1"></p>
                                </label>
                            </div>
                            <div class="col-lg-6">
                                <label class="select" for="slct">
                                    <span class="disabled-label">Your Phone Number *</span>
                                    <input type="tel" class="custominput12" name="contact_no" placeholder="Enter Phone No" id="assessment_phone1" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');" maxlength="10" />
                                    <p class="error m-0" id="phone_err1"></p>
                                </label>
                            </div>
                            <div class="col-lg-6">
                                <label class="select" for="slct">
                                    <span class="disabled-label">Date of Birth *</span>
                                    <input type="date" class="custominput12" id="dobs" name="dob" />
                                    <p class="error m-0" id="dob_err1"></p>
                                </label>
                            </div>
                            <div class="col-lg-6">
                                <label class="select" for="slct">
                                    <span class="disabled-label">Gender *</span>
                                    <select id="gender1" name="gender" class="custominput12">
                                        <option value="">--Select--</option>
                                        <option value="M">Male</option>
                                        <option value="F">Female</option>
                                        <option value="T">Transgender</option>
                                    </select>
                                    <p class="error m-0" id="gender_err1"></p>
                                </label>
                            </div>
                            <div class="col-lg-6">
                                <label class="select" for="slct">
                                    <span class="disabled-label">Assessment *</span>
                                    <select name="assessment_type" id="assessment_type1" class="custominput12">
                                        <option value="">--Select--</option>
                                        <option value="Doctor Consultation">Doctor Consultation</option>
                                        <option value="Dental & Vision Care">Dental & Vision Care</option>
                                        <option value="Mental Wellness">Mental Wellness</option>
                                    </select>
                                    <p class="error m-0" id="type_err1"></p>
                                </label>
                            </div>
                            <div class="col-lg-12">
                                <label class="select" for="slct">
                                    <span class="disabled-label">Write a Message *</span>
                                    <textarea name="message" class="custominput12" id="assessment_message1" cols="30" rows="4"></textarea>
                                    <p class="error m-0" id="msg_err1"></p>
                                </label>
                            </div>
                            <div class="col-lg-12">
                                <button type="submit" class="btn btn-primary w-100" id="btn_assesment1">Submit</button>
                            </div>
                        </div>
                    </div>
                </form>
...
$(document).ready(function() {
    ...
    var maxDate = year + '-' + ((''+month).length<2 ? '0' : '') + month + '-' + ((''+day).length<2 ? '0' : '') + day;
    $("#dobs").attr("max", maxDate);

    $(document).on("click", "#btn_assesment1", function () {
        var url = "<?=base_url()?>employee/add_doctor_assessment_data";
        var email = $("#assessment_email1").val();
        var mobile = $("#assessment_phone1").val();
        var type = $("#assessment_type1").val();
        if ($("#assessment_name1").val() == "") {
            $("#name_err1").html("Please enter your name").show();
            return false;
        }
        if (email == "") {
            $("#email_err1").html("Please enter your email").show();
            return false;
        }
        if (IsEmail(email) == false) {
            $("#email_err1").html("Please enter valid email").show();
            return false;
        }
        if (mobile == "") {
            $("#phone_err1").html("Please enter your mobile no").show();
            return false;
        }
        if (mobile.length != 10) {
            $("#phone_err1").html("Please enter correct mobile no").show();
            return false;
        }

        if ($("#dobs").val() == "") {
            $("#dob_err1").html("Please select your D.O.B").show();
            return false;
        }

        if ($("#gender1").val() == "") {
            $("#gender_err1").html("Please select gender").show();
            return false;
        }
        if (type == "") {
            $("#type_err1").html("Please select assessment").show();
            return false;
        }

        if ($("#assessment_message1").val() == "") {
            $("#msg_err1").html("Please enter message").show();
            return false;
        }
        $.ajax({
            url: url,
            type: "post",
            data: "name=" + $("#assessment_name1").val() + "&email=" + email + "&dob=" + $("#dobs").val() + "&gender=" + $("#gender1").val() + "&contact_no=" + mobile + "&type=" + $("#assessment_type1").val() + "&message=" + $("#assessment_message1").val() + "&tbl=" + "tbl_doctor_assessment",
            dataType: "json",
    ...
    $(".custominput12").bind("change", function () {
        var email = $("#assessment_email1").val();
        var mobile = $("#assessment_phone1").val();
        var type = $("#assessment_type1").val();
        if ($("#assessment_name").val() == "") {
