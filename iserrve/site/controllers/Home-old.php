...
use PHPMailer\PHPMailer\Exception;
	...
	function index()
	{	
		$data['all_category'] = $this->home_model->all_category();
		$data['get_product_detail'] = $this->home_model->get_product_indexPage();
		$this->load->view('index',$data);
	}

	function get_quote($page_url)
	{
		$data['get_product_detail'] = $this->home_model->get_product_indexPage();
		$data['get_product_detail'] = $this->home_model->get_product_details_page_url($page_url);
	...
	{ 	
		$data['L_strErrorMessage'] = ''; 
	        ...
	        //redirect($this->config->item('http_host'));
	...
	{ 	
      ...
      if($this->input->post("action") == "get_quote_12")
			{    		

				$this->home_model->add_qoutePopup($content);	
				$this->session->set_flashdata('L_strsucessMessage','Get Qoute Submitted Successfully!');
				
				if($this->input->post("ri_redirect_gt")=="1") { 	
					redirect($this->config->item('http_host').'thankyou');
				}
			}
			//$this->load->view('get-quote',$data);
	...
	function product_details($cat_page_url,$page_url){
    $data['get_category_detail'] = $this->home_model->get_category_page_url($cat_page_url);
    $data['get_product_detail'] = $this->home_model->get_product_details_page_url($page_url);
    $this->load->view('index',$data);
	}
