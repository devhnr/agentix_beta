...
/* 
		echo "sdsd<pre>";print_r($c);echo "</pre>";
		...
		$coverage_new_array = [];
           		...
           		foreach($all_coverage as $coverage_details){
					...
					//echo "<pre>";print_r($coverage_details);echo "</pre>";exit;
					
					$coverage_name_array = $coverage_details->name;
					$coverage_id_array = $coverage_details->id;
            ...
            // } 
			if($coverage_details->id == $coverage_id){
				$styleDiv = "display:block";
			}else{
				$styleDiv = "display:none";
			}
 ...
 //                 </div>';
	...
	function send_mail_popup(){
		...
		$data_pdf['coverage_type'] = $_POST['type_coverage'];
		$data_pdf['email'] = $_POST['email'];
		
				$html = $this->load->view('success_mail_pdf_new', $data_pdf, true);
				...
				$name_file = $stripped = str_replace(' ', '_', $this->session->userdata('company'));
				...
				$data_new['username'] = $this->session->userdata('company');
				$message = $this->load->view('sucess_mail',$data_new,true);
				...
				redirect($this->config->item('base_url').'risk-assessment');
	 ...
	 function test_new_pdf(){
        
        //echo "dev";exit;
        
        
        
        $data_pdf['username'] = 'test';

				$html = $this->load->view('success_mail_pdf_new', $data_pdf, true);exit;
    }
