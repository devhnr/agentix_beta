<?php
	...
	{	 
			...
			if(is_numeric($id))
			{
				$result = $this->banner_model->get_system($id);  
			...
			if($_FILES['image']['name'] != ''){
						...
						$pathAndName = $this->config->item('upload')."banner/".$fileName; 
						$moveResult = move_uploaded_file($fileTmpLoc, $pathAndName);
						$tmp_path = $this->config->item('upload')."banner/".$fileName;
						$image_thumb= $this->config->item('upload')."banner/large/".$fileName; 
						...
						$tmp_path = $this->config->item('upload')."banner/".$fileName;
						$image_thumb= $this->config->item('upload')."banner/medium/".$fileName; 
						...
						}else
						{
							...
							redirect($this->config->item('base_url').'banner/edit/1');
				}
