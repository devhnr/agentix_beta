<?php
	class Msme_edge_model extends CI_Model {
	private $_data = array();
	...
	function add($content) 
	{
		...
		$this->_data = $data;
		if ($this->db->insert('msme_edge', $this->_data))	
		{		
			return true; 
		} 
		else 
	...
	function edit($id, $content) 
	{
		...
		$this->_data = $data;
		$this->db->where('id', $id);
		if ($this->db->update('msme_edge', $this->_data))	{
			return true;
		} else {
			return false;
		}
    ...
    function lists($num, $offset, $content) 
	{
		...
		$sql = "SELECT * FROM msme_edge where id <> 0 ";
		...
		$sql_count = "SELECT * FROM  msme_edge  WHERE id <> 0";
		if($content['categoryname'] !='')
		{
...
function is_already_exist_edit($user,$id)
	...
	function updateorder($id,$val){
		...
		$this->db->where('id',$id);
		return $this->db->update('msme_edge', $content);	
	}	
}
?>