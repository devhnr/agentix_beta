<?php
	class Product_strength_model extends CI_Model {
	private $_data = array();
	...
	function add($content) 
	{
		...
		$this->_data = $data;
		if ($this->db->insert('product_strength', $this->_data))	
		{		
			return true; 
		} 
		else 
	...
	function edit($id, $content) 
	{
		...
		$this->_data = $data;
		$this->db->where('id', $id);
		if ($this->db->update('product_strength', $this->_data))	{
			return true;
		} else {
			return false;
		}
    ...
    function lists($num, $offset, $content) 
	{
		...
		$sql = "SELECT * FROM product_strength where id <> 0 ";
		...
		$sql_count = "SELECT * FROM  product_strength  WHERE id <> 0";
		if($content['categoryname'] !='')
		{
	...
	{
 		$this->db->where('id = ',$id);
		if ($this->db->delete('product_strength'))	{
			return true;
		} else {
			return false;
		}
...
function is_already_exist_edit($user,$id)
	...
	function updateorder($id,$val){
		...
		$this->db->where('id',$id);
		return $this->db->update('product_strength', $content);	
	}	
}
?>