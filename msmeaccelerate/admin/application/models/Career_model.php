<?php
	...
	function add($content) 
	{
		...
		$this->_data = $data;
		if ($this->db->insert('career', $this->_data))	
		{		
			return true; 
		} 
		else 
	...
	function edit($id, $content) 
	{
		...
		$this->_data = $data;
		$this->db->where('id', $id);
		if ($this->db->update('career', $this->_data))	{
			return true;
		} else {
			return false;
		}
    ...
    function lists($num, $offset, $content) 
	{
		...
		$sql = "SELECT * FROM career where id <> 0 ";
		...
		$sql_count = "SELECT * FROM  career  WHERE id <> 0";
		if($content['categoryname'] !='')
		{
...
function is_already_exist_edit($user,$id)
	...
	function updateorder($id,$val){
		...
		$this->db->where('id',$id);
		return $this->db->update('career', $content);	
	}	
}
?>