<?php
	...
	function add($content) 
	{
		...
		$this->_data = $data;
		if ($this->db->insert('agent', $this->_data))	
		{		
			return true; 
		} 
		else 
	...
	function edit($id, $content) 
	{
		...
		$this->_data = $data;
		$this->db->where('id', $id);
		if ($this->db->update('agent', $this->_data))	{
			return true;
		} else {
			return false;
		}
    ...
    function lists($num, $offset, $content) 
	{
		...
		$sql = "SELECT * FROM agent where id <> 0 ";
		...
		$sql_count = "SELECT * FROM  agent  WHERE id <> 0";
		if($content['categoryname'] !='')
		{
...
function is_already_exist_edit($user,$id)
	...
	function updateorder($id,$val){
		...
		$this->db->where('id',$id);
		return $this->db->update('agentname', $content);	
	}	
}
?>