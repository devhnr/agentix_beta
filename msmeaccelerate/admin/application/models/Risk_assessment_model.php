<?php
...

	{
		...
		$this->_data = $data;
		if ($this->db->insert('risk_assessment', $this->_data))	
		{		
			return true; 
		} 
		else 
	...
	function edit($id, $content) 
	{
		...
		$this->_data = $data;
		$this->db->where('id', $id);
		if ($this->db->update('risk_assessment', $this->_data))	{
			return true;
		} else {
			return false;
		}
	...
	{
		$query = "update risk_assessment set set_home = '".$value."' where id = '".$pid."'";
    ...
    function lists($num, $offset, $content)
	{
		...
		$sql = "SELECT * FROM risk_assessment where id <> 0 ";
		...
		$sql_count = "SELECT * FROM  risk_assessment  WHERE id <> 0";
...

	{
		...
		if ($this->db->delete('risk_assessment'))	{
	...
	function get_risk_assessment($id)
	{
		$this->db->where('id',$id);
	...
	function updateorder($id,$val){
		...
		return $this->db->update('risk_assessment', $content);	
